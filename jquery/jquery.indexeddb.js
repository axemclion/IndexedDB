		...
		linq4idb : function(config) {
			...
			var promise = {
				...
				versionTransaction : function(dbPromise, version) {
					return $.Deferred(function(dfd) {
						$.when(dbPromise).then(function(db) {
							// console.debug("Version Change Transaction Promise
							// started", db, version);
							var req = db.setVersion(version || (isNaN(parseInt(db.version, 10)) ? 0 : parseInt(db.version, 10) + 1));
							req.onsuccess = function() {
								// console.debug("Version Change Transaction
								// Promise completed", req.result);
				...
				transaction : function(dbPromise, objectStoreNames, transactionType) {
					return $.Deferred(function(dfd) {
						...
						$.when(dbPromise).then(function(db) {
							// console.debug("Transaction Promise started", db,
							// objectStoreNames, transactionType);
							try {
								...
								// console.debug("Transaction Promise
				...
				objectStore : function(transactionPromise, objectStoreName, createOptions) {
					return $.Deferred(function(dfd) {
						$.when(transactionPromise).then(function(transaction) {
							// console.debug("ObjectStore Promise started",
							// transactionPromise, objectStoreName,
							// createOptions);
							try {
								if (createOptions) {
								...
								// console.debug("ObjectStore Promise
								// completed", objectStore);
								dfd.resolve(objectStore);
							} catch (e) {
								// console.debug("Error in Object Store
								// Promise", e);
				...
				index : function(indexName, objectStorePromise) {
					return $.Deferred(function(dfd) {
						$.when(objectStorePromise).then(function(objectStore) {
							// console.debug("Index Promise started",
							// objectStore)
							try {
								var index = objectStore.index(indexName + "-index");
								// console.debug("Index Promise compelted",
								// index);
								dfd.resolve(index);
							} catch (e) {
								...
								// console.debug("Index Promise requires version
								// change");
								$.when(promise.versionTransaction(promise.db(name))).then(function(transaction) {
									// console.debug("Index Promise version
									// change transaction started",
									// transaction);
									try {
										...
										// console.debug("Index Promise
										// completed", index);
										dfd.resolve(index);
									} catch (e) {
										// console.debug("Index Promise Failed",
										// e);
				...
				cursor : function(sourcePromise, range, direction) {
					return $.Deferred(function(dfd) {
						$.when(sourcePromise).then(function(source) {
							...
							req.onsuccess = function() {
								// console.debug("Cursor Promise completed",
								// req);
								dfd.resolve(req);
							};
							req.onerror = function(e) {
								// console.debug("Cursor Promise error", e,
								// req);
								dfd.reject(e, req);
							};
						});
			...
			var objectStorePromise = function(dbName, objectStoreName, transactionType) {
				return $.Deferred(
						function(dfd) {
							...
							$.when(promise.objectStore(promise.transaction(dbPromise, objectStoreName, transactionType), objectStoreName)).then(
			...
			return {
				"open" : function(dbName) {
					return {
						"setVersion" : function(versionNumber, callback){
